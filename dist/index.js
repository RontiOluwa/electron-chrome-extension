...
const resolveExtensionPath = (extension, uri) => {
  ...
  const resPath = path__WEBPACK_IMPORTED_MODULE_1__.join(extension.path, relpath);
...
let ResizeType = /*#__PURE__*/function (ResizeType) {
...
/* harmony export */ });
var CookieStoreID = /*#__PURE__*/function (CookieStoreID) {
...
const createCookieDetails = cookie => ({
  ...cookie,
  ...
  constructor(ctx) {
    ...
    this.clear = ({
      ...
      if (this.registry.has(notificationId)) {
        ...
        (_this$registry$get = this.registry.get(notificationId)) === null || _this$registry$get === void 0 || _this$registry$get.close();
      }
    };
    this.create = async ({
      extension
      ...
      if (this.registry.has(notificationId)) {
        ...
        (_this$registry$get2 = this.registry.get(notificationId)) === null || _this$registry$get2 === void 0 || _this$registry$get2.close();
      }
      let icon;
      if (opts.iconUrl) {
        var _url;
...
class TabsAPI {
  ...
  observeTab(tab) {
    const tabId = tab.id;
    const updateEvents = ['page-title-updated',
    // title
    'did-start-loading',
    // status
    'did-stop-loading',
  ...
  async create(event, details = {}) {
    ...
    const tab = await this.ctx.store.createTab({
      ...details,
      url
    }, event);
    if (!tab) return;
  ...
  async query(event, info = {}) {
    ...
    const filteredTabs = Array.from(this.ctx.store.tabs).map(this.getTabDetails.bind(this)).filter(tab => {
      ...
      if (isSet(info.currentWindow)) {
        ...
        if (info.currentWindow && (currentWin === null || currentWin === void 0 ? void 0 : currentWin.id) !== tab.windowId) {
          // The workaround
          if ((lastFocusedWindow === null || lastFocusedWindow === void 0 ? void 0 : lastFocusedWindow.id) === tab.windowId) {
            return true;
          }
          return false;
        }
...
const debug = __webpack_require__(/*! debug */ "./node_modules/debug/src/index.js")('electron-chrome-extensions:webNavigation');
...
class WebNavigationAPI {
  constructor(ctx) {
    ...
    this.onCreatedNavigationTarget = (event, url, isInPlace, isMainFrame, frameProcessId, frameRoutingId) => {
      ...
      const details = {
        sourceTabId: tab === null || tab === void 0 ? void 0 : tab.id,
    ...
    this.onFinishLoad = (event, isMainFrame, frameProcessId, frameRoutingId) => {
      const frame = getFrame(frameProcessId, frameRoutingId);
      const tab = event.sender;
      const url = tab === null || tab === void 0 ? void 0 : tab.getURL();
      const details = {
        frameId: getFrameId(frame),
        parentFrameId: getParentFrameId(frame),
        processId: frameProcessId,
...
class PopupView {
  ...
  setSize(rect) {
    ...
    (_this$browserWindow3 = this.browserWindow) === null || _this$browserWindow3 === void 0 || _this$browserWindow3.setBounds({
...
class ExtensionStore extends events__WEBPACK_IMPORTED_MODULE_1__.EventEmitter {
  ...
  getLastFocusedWindow() {
    return this.lastFocusedWindowId ? this.findWindowById(this.lastFocusedWindowId) || null : null;
  }
  addWindow(window) {
  ...
  removeTab(tab) {
    ...
    (async (_this$impl$getTabbedB, _this$impl2) => {
